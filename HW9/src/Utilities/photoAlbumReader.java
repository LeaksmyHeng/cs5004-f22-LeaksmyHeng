...
public class photoAlbumReader{
  ...
  public static <T> T parseFile(Readable readable, photoAlbumBuilder<T> builder) {
    Objects.requireNonNull(readable, "Need to be not null for the source.");
    Objects.requireNonNull(builder, "Need to be not null for photoAlbumReader.");
    try {
      ...
      while (myReader.hasNext()) {
        ...
        switch (word) {
          case "shape" -> readShape(myReader, builder);
          case "snapshot" -> readSnapShot(myReader, builder);
          case "move" -> readChangePosition(myReader, builder);
          case "resize" -> readChangeShapeSize(myReader, builder);
          case "color" -> readChangeColor(myReader, builder);
          case "remove" -> readRemove(myReader, builder);
          default -> throw new IllegalStateException("Unexpected keyword: " + word + myReader.nextLine());
        }
      }
      return builder.build();
    }
    catch (NullPointerException e) {
      System.out.println("Error occur while trying to parse file.");
      e.printStackTrace();
    }
    return builder.build();
  }
