...
// *** YOU Write the BoxerComparator instantiated in the line above ***
    ...
    List<Boxer> sortedByWeight = boxers.stream().sorted(byWeight).collect(Collectors.toList());
    System.out.println(sortedByWeight);

    List<Boxer> heavyweights = sortedByWeight.stream().filter(boxer -> boxer.weightClass == WEIGHT_CLASS.HEAVY).collect(Collectors.toList());
